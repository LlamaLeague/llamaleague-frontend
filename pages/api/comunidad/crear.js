...
export default async function handler(req, res) {
  ...
  if (!/^[a-z0-9-]{2,20}$/.test(tag))                     return res.status(400).json({ error: 'Tag invalido (solo letras minúsculas, números y guiones, 2-20 chars)' })
  if (!channel_url?.trim())                                 return res.status(400).json({ error: 'La URL del canal es obligatoria' })
  if (!['open','subs_only','whitelist'].includes(access_mode)) return res.status(400).json({ error: 'Modo de acceso invalido' })

  // Verificar tag único
  const { data: tagExisting } = await supabaseAdmin
    .from('communities').select('id').eq('tag', tag).single()
  if (tagExisting) return res.status(400).json({ error: 'Ese tag ya está en uso, elige otro' })

  // Verificar que el streamer no tenga ya una comunidad
  const { data: miComunidad } = await supabaseAdmin
